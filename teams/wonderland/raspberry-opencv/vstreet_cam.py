...
def overlay_on_image(display_image, filtered_objects):
    ...
    for obj_index in range(len(filtered_objects)):
        ...
        if (filtered_objects[obj_index][8] > GN_PROBABILITY_MIN):
            ...
            object_name = filtered_objects[obj_index][7]
            object_probability = filtered_objects[obj_index][8]
        else:
            label_text = filtered_objects[obj_index][0] + ' : %.2f' % filtered_objects[obj_index][5]
            object_name = filtered_objects[obj_index][0]
            object_probability = filtered_objects[obj_index][5]
        ...
        if object_name in objects_new:
            objects_new[object_name].append({
                "height": box_bottom - box_top,
                "width": box_right - box_left,
                "probability": object_probability
            })
        else:
            objects_new[object_name] = [{
                "height": box_bottom - box_top,
                "width": box_right - box_left,
                "probability": object_probability
            }]

    objects = copy.deepcopy(objects_new)
    # print(objects)
