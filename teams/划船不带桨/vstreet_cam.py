...
def overlay_on_image(display_image, filtered_objects):
    ...
    for obj_index in range(len(filtered_objects)):
        ...
        if max_bnd_area < (box_bottom-box_top)*(box_right-box_left):
            max_bnd_area = (box_bottom-box_top)*(box_right-box_left)
            max_id = obj_index
    ...
    # inserted codes
    # global old_real_cx, old_real_cy, old_box_top, old_box_bottom, old_box_left, old_box_right

    if max_id < 0:
        cmd_str = "no_box,-1"

    else:
        max_center_x = int(filtered_objects[max_id][1])
        max_center_y = int(filtered_objects[max_id][2])
        max_half_width = int(filtered_objects[max_id][3])//2 + DISPLAY_BOX_WIDTH_PAD
        max_half_height = int(filtered_objects[max_id][4])//2 + DISPLAY_BOX_HEIGHT_PAD
        max_box_left = max(max_center_x - max_half_width, 0)
        max_box_top = max(max_center_y - max_half_height, 0)
        max_box_right = min(max_center_x + max_half_width, source_image_width)
        max_box_bottom = min(max_center_y  + max_half_height, source_image_height)
